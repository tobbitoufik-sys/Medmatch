...
const SECTION_DEFINITIONS: SectionDefinition[] = [
  {
    ...
    description: "Priorisieren Sie die Stationen, die im Lebenslauf zuerst sichtbar sein sollen."
  },
  {
    key: "education",
    title: "Ausbildung",
    description: "Ordnen Sie Abschlusse, Universitat und Datumsangaben klar fur Recruiter."
  },
  {
    key: "qualifications",
    title: "Medizinische Qualifikationen",
    description: "Zusatzliche Qualifikationen und fachliche Schwerpunkte kompakt verwalten."
  },
  {
    key: "languages",
    title: "Sprachen",
    description: "Sprachniveaus sortieren und nur die relevanten Eintrage aktiv lassen."
  },
  {
    key: "trainings",
    title: "Fortbildungen",
    description: "Aktuelle Fortbildungen sichtbar machen und weniger wichtige ausblenden."
  },
  {
    key: "additional_sections",
    title: "Zusatzliche Abschnitte",
    description: "Weitere relevante Informationen bei Bedarf gesondert hervorheben."
  }
];
...
export function CvLayoutEditorV2({
  cvModel,
  experiences,
  ...
  const availableSectionKeys = useMemo(() => {
    ...
    if (cvModel.qualifications.length && !cvModel.workExperience.length) {
  ...
  const headerItems = useMemo<HeaderItem[]>(
    () => [
      { id: "header-photo", label: "Foto" },
      { id: "header-full-name", label: "Vollstandiger Name" },
      ...(cvModel.header.subtitle
        ? [{ id: "header-subtitle", label: "Berufsbezeichnung" }]
        : []),
  ...
  const summaryContactItems = useMemo(
    () =>
      cvModel.personalInfo.items
        .map((item, index) => ({
          id: `personal-info-item-${index}`,
          value: item
        }))
        .filter((item) => itemVisibility[item.id] !== false),
    [cvModel.personalInfo.items, itemVisibility]
  );

  const summaryAddress =
    cvModel.personalInfo.address_line && itemVisibility["personal-info-address-line"] !== false
      ? cvModel.personalInfo.address_line
      : null;
          ...
          <div className="space-y-3">
            <div>
              ...
              <h2 className="mt-1 text-lg font-semibold text-slate-950">
                Vorlage und Aktionen
              </h2>
            </div>

            <div className="w-full max-w-sm">
              <label className="mb-2 block text-sm font-medium text-slate-700" htmlFor="cv-template-select-v2">
                Vorlage
              </label>
              <select
                id="cv-template-select-v2"
                value={selectedTemplate}
                onChange={(event) => updateSelectedTemplate(event.target.value as CvTemplateKey)}
                className="h-11 w-full rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-700 outline-none focus-visible:ring-2 focus-visible:ring-slate-300"
              >
      ...
      <section className="space-y-3 rounded-[1.5rem] border border-slate-200 bg-slate-50/65 p-4">
        ...
        <div className="space-y-2">
          {headerItems.map((item) => {
            const included = itemVisibility[item.id] !== false;
